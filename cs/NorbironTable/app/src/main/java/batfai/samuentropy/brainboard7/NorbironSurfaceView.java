...
public class NorbironSurfaceView extends android.view.SurfaceView implements Runnable {
    ...
    private Nodes nodes;
    public static java.util.List<NeuronBox> nodeBoxes = new java.util.concurrent.CopyOnWriteArrayList<>();
    ...
    public void spawnNode(android.os.Bundle bundle){
        if(bundle != null){
    ...
    public void addNode(int i, float x, float y)
    {
        nodeBoxes.add((NeuronBox) nodes.get(i).clone());
    ...
    private long eventFinishTime = 1000;
    ...
    public boolean onTouchEvent(android.view.MotionEvent event) {
        ...
        if (event.getAction() == android.view.MotionEvent.ACTION_DOWN) {
            ...
            prevEventTime = eventFinishTime;
            eventFinishTime = event.getEventTime();
            ...
            if (eventFinishTime - prevEventTime < 250 &&
                    Math.abs(TouchX-prevTouchX) < 25 &&
                    Math.abs(TouchY-prevTouchY) < 25){
                nodeBoxes.remove(selNb);
            }
        }
